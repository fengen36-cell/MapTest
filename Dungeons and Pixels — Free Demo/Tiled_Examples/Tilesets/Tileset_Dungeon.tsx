<?xml version="1.0" encoding="UTF-8"?>
<tileset version="1.10" tiledversion="1.11.0" name="Tileset_Dungeon" tilewidth="32" tileheight="32" tilecount="60" columns="12">
 <image source="../../Tilesets/Tileset_Dungeon.png" width="384" height="160"/>
 <tile id="43">
  <animation>
   <frame tileid="43" duration="250"/>
   <frame tileid="44" duration="250"/>
   <frame tileid="45" duration="250"/>
   <frame tileid="46" duration="250"/>
  </animation>
 </tile>
 <tile id="55">
  <animation>
   <frame tileid="55" duration="250"/>
   <frame tileid="56" duration="250"/>
   <frame tileid="57" duration="250"/>
   <frame tileid="58" duration="250"/>
  </animation>
 </tile>
</tileset>
